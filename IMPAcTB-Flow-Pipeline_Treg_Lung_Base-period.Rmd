...
ggplot(t_cell_activationHIST, aes(x = successes, fill = group)) +
  ...
  scale_fill_viridis_d(name = "Vaccine") +  # Add legend title
  labs(y = "Frequency", x = "CD19+ Distribution of Success") +  
  #facet_wrap(~ group, labeller = my_labeller) +  # Use custom labeller
  facet_wrap(~ group + day, ncol = 3) +
  theme_minimal() +
  theme(
    ...
    legend.position = "none") +
  scale_x_continuous(breaks = seq(0, max(t_cell_activationHIST$successes), by = 10000))
...
t_cell_activation$sex <- relevel(t_cell_activation$sex, ref = "male")
