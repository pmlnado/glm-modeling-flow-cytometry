...
```{r}
#cleaned_data$sex <- relevel(cleaned_data$sex, ref = "female")

# Fitting the GLM with 'male' as the reference level for 'sex'
cleaned_datal_mod2 <- glm(cbind(successes, failures) ~ group * day + sex,
              family = quasibinomial(link = "logit"), data = cleaned_data)

# sex as interaction
cleaned_datal_mod1 <- glm(cbind(successes, failures) ~ group * day * sex,
              family = quasibinomial(link = "logit"), data = cleaned_data)

summary_l_mod1cleaned_data <- summary(cleaned_datal_mod1)
summary_l_mod2cleaned_data <- summary(cleaned_datal_mod2)
summary_l_mod1cleaned_data
summary_l_mod2cleaned_data

drop_in_dev1cleaned_data <- anova(cleaned_datal_mod2, cleaned_datal_mod1, test = "Chisq")

drop_in_dev1cleaned_data
drop_in_dev1

1- (cleaned_datal_mod1$deviance/cleaned_datal_mod1$null.deviance)
1- (cleaned_datal_mod2$deviance/cleaned_datal_mod2$null.deviance)
...
#needs work
res <- residuals(l_mod1, type = "deviance")
...
outlier_indices <- which(res < lower_bound | res > upper_bound)

# Subset your dataset: remove those outliers
t_cell_data_cleaned <- t_cell_data[-outlier_indices, ]
...
coefficients_table <- summary_l_mod1$coefficients
...
CoefficientsandConfidence_Intervals_Treg <- ggplot(coefficients_df, aes(x = reorder(Variable, Estimate), y = Estimate, ymin = Estimate - 1.96 * SE, ymax = Estimate + 1.96 * SE)) +
  ...
  coord_flip() +
  labs(title = "CD19+ Estimated Effects",
       x = "Variable",
...
plotSex_day14 <- effect_plot(l_mod1, pred = sex, at = list(day = "14"), interval = TRUE, y.label = "Probability of Activated T-cells at Day 14")
plotSex_day56 <- effect_plot(l_mod1, pred = sex, at = list(day = "56"), interval = TRUE, y.label = "Probability of Activated T-cells at Day 56")
plotSex_day90 <- effect_plot(l_mod1, pred = sex, at = list(day = "90"), interval = TRUE, y.label = "Probability of Activated T-cells at Day 90")

# Generate effect plots for different levels of 'day' and 'group'
effect_plot(l_mod1, pred = sex, at = list(day = "14", group = "BCG+ID93-GLA-SE"), interval = TRUE, y.label = "Probability of Activated T-cells at Day 14")
effect_plot(l_mod1, pred = sex, at = list(day = "56", group = "BCG+ID93-GLA-SE"), interval = TRUE, y.label = "Probability of Activated T-cells at Day 56")
effect_plot(l_mod1, pred = sex, at = list(day = "90", group = "BCG+ID93-GLA-SE"), interval = TRUE, y.label = "Probability of Activated T-cells at Day 90")
...
effect <- effect_plot(l_mod1, pred = "sex", interval = TRUE, y.label = "Probability of CD19+ B-cells")
