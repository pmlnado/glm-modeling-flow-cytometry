...
t_cell_activationHIST %>%
  summarize(
    mean_successes = mean(successes),
    median_successes = median(successes),
    min_successes = min(successes),
    max_successes = max(successes)
  )
...
mosaicplot(~ sex + group + day, data = t_cell_activationHIST, color = TRUE,
       labeling_args = list(gp_labels = gpar(fontsize = 8)),
       xlab = "Sex",
       ylab = "Group",
       main = "Mosaic Plot of Sex, Group, and Day")
...
plot(fitted_values, dev_res, main = "Residuals vs. Fitted", 
...
Treg_probability_heatmap <- ggplot(results, aes(x = day, y = group)) +
  ...
  scale_fill_viridis_c(name = "Probability of Activated T-cells", labels = function(x) paste0(x, "%")) +# Add 5 to each label
  theme_minimal() +
  ...
  theme(
    ...
    axis.title.y = element_text(color = "black", size = 1.3 * rel(1)),
    legend.text = element_text(color = "black", size = 11),
    legend.title = element_text(color = "black", size = 11))  # Adjust the font size as needed
...
Treg_activationPLOT <- t_cell_activationPLOT  %>%
  ...
  labs(title = "Activated T Cells in Mtb Infected Lung", x = "Days Post Infection", y = "Percentage of Live", color = "Vaccine") +
  #guides(fill = "none", shape = guide_legend(title = "Vaccine")) +
  theme_bw() + 
  theme(
    axis.text.x = element_text(angle = 45, hjust = 1, color = "black", size = 1.2 * rel(1)),
    axis.text.y = element_text(color = "black", size = 1.2 * rel(1)),
    legend.text = element_text(color = "black", size = .9 * rel(1)),
    legend.title = element_text(color = "black", size = 1 * rel(1))) +
