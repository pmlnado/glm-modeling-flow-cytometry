...
files <- list.files(path = "/Volumes/rstor-henao_lab/Pablo/IMPAcTB-Flow-pipeline/Option-1_Treg", pattern = "*.csv", full.names = TRUE)
...
t_cell_activation <- tidy_Df1 %>%
  dplyr::filter(cell_types == "live leukocytes" | cell_types == "CD3+ CD4+ CD8- CD44+ CD62L-") 

t_cell_activation <- t_cell_activation %>%
  pivot_wider(names_from = cell_types, values_from = `cell type count`) %>%
  rename(trials = "live leukocytes", successes = "CD3+ CD4+ CD8- CD44+ CD62L-") %>%
...
drop_in_dev2 <- anova(l_mod2, l_mod1, test = "Chisq")
...
ggplot(predictions, aes(x = x, y = predicted, group = group, color = group, fill = group)) +
  ...
  facet_wrap(~ facet) +
  theme_bw() +
  scale_y_continuous(labels = scales::percent, breaks = seq(0, 1, by = 0.1), limits = c(0, .22))
